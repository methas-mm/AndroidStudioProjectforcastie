...
import cz.martykan.forecastie.activities.MainActivityforecastie;
...
public abstract class AbstractWidgetProvider extends AppWidgetProvider {
    ...
    protected Weather parseWidgetJson(String result, Context context) {
        try {
            MainActivityforecastie.initMappings();
            ...
            if (lastUpdateTimeInMillis < 0) {
                ...
                lastUpdate = context.getString(R.string.last_update_widget, MainActivityforecastie.formatTimeWithDayIfNotToday(context, lastUpdateTimeInMillis));
            }
            ...
            widgetWeather.setWind(context.getString(R.string.wind) + ": " + new DecimalFormat("0.0").format(wind) + " " + localize(sp, context, "speedUnit", "m/s")
                    + (widgetWeather.isWindDirectionAvailable() ? " " + MainActivityforecastie.getWindDirectionString(sp, context, widgetWeather) : ""));
    ...
    protected String localize(SharedPreferences sp, Context context, String preferenceKey,
        ...
        return MainActivityforecastie.localize(sp, context, preferenceKey, defaultValueKey);
    }
